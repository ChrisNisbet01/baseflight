...
static void pidRewrite(void)
{
    ...
    for (axis = 0; axis < 3; axis++) {
        ...
        if (axis == 2) { // YAW is always gyro-controlled (MAG correction is applied to rcCommand)
            ...
            errorAngle = (constrain(rcCommand[axis] + GPS_angle[axis], -((int)mcfg.max_angle_inclination), mcfg.max_angle_inclination) - angle[axis] + cfg.angleTrim[axis]) / 10; // 16 bits is ok here
