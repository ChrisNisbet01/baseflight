...
static void adxl345Read(sensor_data_t *accelData);
...
static void adxl345Read(sensor_data_t *accelData)
{
    ...
    if (useFifo) {
        ...
        do {
            ...
            x += (sensor_data_t)(buf[0] + (buf[1] << 8));
            y += (sensor_data_t)(buf[2] + (buf[3] << 8));
            z += (sensor_data_t)(buf[4] + (buf[5] << 8));
